...
public class PrimeraClasse {
	...
	/**
	*	Esse é o método main, o ponto de entrada para o seu código Java.
	*	@param args
	*/
	public static void main(String[]args) {
		...
		//usa divisão de inteiros e ignora o que sobra
		...
		//exponenciação
		//Você tem que usar métodos para fazer cálculos com esponenciais
		...
		//Metodo de String para converter para maiúsculo
		...
		//Checagem de condicional se x é par, usando o operador % (resto de divisão)
		System.out.println();
		System.out.println("x: "+ x);
		...
		//operadores boolean
		//&& (and) - verdadeiro se ambos os operadores são verdadeiros
		System.out.println();
		if (e > 2 && e < f) {
		...
		if (f > e || f > g1) {
			System.out.println(g1 + " não é igual a 2.6");
		}
		...
		/*
		 * Casting (Transformação do tipo da variável)
		 */
		...
		//Cast para objeto e depois chama o método getClass()
		...
		int inteiro = scan.nextInt(); //recebe o próximo input como um numero inteiro
		System.out.println("Seu número é: "+ inteiro);
		
		//imprime a table de multiplicação até 10 de inteiro
		for (int k = 1; k <= 10; k++) {
			//print k * inteiro
		...
		String myStr = scan.next(); //Recebe o próximo input como string
		
		//Imprime cada char em myStr separadamente
		for (int u = 0; u < myStr.length(); u++) {
			//imprime o char na posição u
